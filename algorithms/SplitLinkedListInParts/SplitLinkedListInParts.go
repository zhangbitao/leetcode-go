...
//  output[0].next = null. The last element output[4] is null, but it's string representation as a ListNode is [].
//
// Example 2:
//
// Input:
...
func splitListToParts(root *ListNode, k int) []*ListNode {
	...
	for p := root; p != nil; {
		...
		l1, l2 := splitListToTwoParts(p, cnt)
		result[index] = l1
		index++
		p = l2
...
func splitListToTwoParts(root *ListNode, n int) (l1, l2 *ListNode) {
	if root == nil || n <= 0 {
		return
	}
